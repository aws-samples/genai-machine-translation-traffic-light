...
const App = ({ signOut }) => {
  ...
  const [source, setSource] = useState(
    "16 Penn State in which redshirt freshman Jim Kelly threw for 280 yards and three touchdowns in his first career start as Miami's quarterback."
  );
  const [translation, setTranslation] = useState(
    "16 Penn State, in dem Rothemd-Neuling Jim Kelly warf 280 Meter und drei Touchdowns in seiner ersten Karriere starten als Miami Quartier zurück."
  );
  const [llm, setLLM] = useState({ label: "Claude 3 Sonnet", value: "claude" });
  ...
  const submitQuery = async () => {
    try {
      ...
      const response = await API.post("api", "/", {
        body: { source, translation, llm, language },
            ...
            <SpaceBetween size="s">
              ...
              <SpaceBetween direction="horizontal" size="xl">
                <Select
                  ...
                  options={[
                    { label: "Claude 3 Sonnet", value: "claude" },
                    { label: "Llama 2 70B", value: "llama" },
                  ]}
                />
                <RadioGroup
              ...
              <SpaceBetween direction="horizontal" size="s">
                <TextContent>
                  <h2>Reason:</h2>
                  <div style={{ display: "block" }}>{reason}</div>
                </TextContent>
              </SpaceBetween>
              <SpaceBetween direction="horizontal" size="s">
                <TextContent>
